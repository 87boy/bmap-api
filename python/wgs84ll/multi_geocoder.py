...
def geocoder(num):
    print "Child process start with pid = %s, num = %s" % (os.getpid(), num)
    ...
    try:
        ...
        try:
            ...
            log.write(current_time + "\tError: first query geocoding_poi fail\n")
            print "Child process break with pid = %s, num = %s" % (os.getpid(), num)
            # 关闭数据库连接
            db.close()
            # 关闭日志文件
            log.close()
    ...
    while (i < max):
        ...
        if (not point[5]) and point[1] and point[2] and point[3] and point[4]:
    ...
    # 关闭日志文件
    log.close()
    print "Child process finish with pid = %s, num = %s" % (os.getpid(), num)
